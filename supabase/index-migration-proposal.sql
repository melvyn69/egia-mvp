-- Final minimal index proposal for Dashboard, Reports, Competitors, and Loyalty.
-- Proposal only: do not apply until reviewed against production query plans.
--
-- Existing migration coverage not repeated here:
-- - business_settings_user_id_idx
-- - google_locations_user_id_idx and google_locations_id_user_id_uidx
-- - google_reviews location/source-time indexes and user/location/status index
-- - competitors unique (user_id, location_id, place_id) and user/location/followed index
-- - loyalty_programs user/location indexes, loyalty_visits indexes, and existing loyalty prefix indexes
--
-- Removed from the broader recommendation:
-- - google_reviews_user_location_create_time_idx because location-specific create_time scans are already covered.
-- - competitors_user_location_scanned_idx because last_scanned_at is not defined in local migrations/schema.
