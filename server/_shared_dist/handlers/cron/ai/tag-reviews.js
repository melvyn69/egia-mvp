...
const upsertAiStatus = async (userId, locationId, value) => {
    await supabaseAdmin.from("cron_state").upsert({
...
const acquireLock = async (userId, locationId) => {
...
const releaseLock = async (userId, locationId) => {
    const key = `lock_ai_tag_v1:${userId}:${locationId}`;
...
const normalizeTopics = (topics) => {
    ...
    for (const topic of topics ?? []) {
        ...
        normalized.push({
            ...
            polarity: typeof topic.polarity === "number"
                ? clamp(topic.polarity, -1, 1)
                : undefined,
            confidence: typeof topic.confidence === "number"
...
const analyzeReview = async (review, requestId, debugInfo) => {
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) {
        throw new Error("Missing OPENAI_API_KEY");
    }
    const model = process.env.OPENAI_MODEL || "gpt-4o-mini";
    const shouldRetry = (status) => [429, 503, 504].includes(status);
    try {
        const body = JSON.stringify({
            ...
            input: [
                {
                    role: "system",
                    content: "Tu es un analyste d'avis Google. Réponds en français uniquement. " +
                ...
                {
                    role: "user",
                    content: "Analyse cet avis client et produis les champs demandés (sentiment, score, résumé, topics). " +
        ...
        try {
            ...
            console.error("[ai-tag]", requestId, "OpenAI JSON parse failed", outputText.slice(0, 300));
            throw error;
        }
        if (debugInfo) {
            debugInfo.outputTextPreview = outputText.slice(0, 200);
...
export default async function handler(req, res) {
    ...
    const processedByLocation = new Map();
    const tagsByLocation = new Map();
    const lastErrorByLocation = new Map();
    ...
    try {
        ...
        const { data: missingCountData, error: missingCountError } = await supabaseAdmin.rpc("ai_tag_candidates_count", {
            p_user_id: null,
            p_location_id: null
        });
        ...
        console.log("[ai-tag]", requestId, "totalMissingInsights", totalMissingInsights);
        ...
        const { data: candidates, error: candidatesError } = await supabaseAdmin.rpc("ai_tag_candidates", {
            p_user_id: null,
            p_location_id: null,
        ...
        console.log("[ai-tag]", requestId, "candidatesFound", candidatesFound);
        if (candidateRows.length > 0) {
            console.log("[ai-tag]", requestId, "candidateRange", candidateRows[0]?.id, candidateRows[candidateRows.length - 1]?.id);
        }
        ...
        if (debugEnabled) {
            debug = {
                ...
                candidateQueryMeta: {
                    filter: "comment not null and length(trim(comment))>0 and no existing insights",
        ...
        for (const locationId of locationIds) {
            const { count: _count } = await supabaseAdmin
                ...
                .neq("comment", "");
            void _count;
            const { data: missingData } = await supabaseAdmin.rpc("ai_tag_candidates_count", {
                p_user_id: null,
        ...
        const candidateRowsByLock = candidateRows.filter((review) => review.location_id ? lockedLocations.has(review.location_id) : false);
        ...
        for (const review of candidateRowsByLock) {
            ...
            const reviewId = typeof review === "object" && review !== null && "id" in review
                ? String(review.id ?? "")
                : "";
            ...
            if (!effectiveUpdateTime) {
                ...
                if (locationId) {
                    errorsByLocation.set(locationId, (errorsByLocation.get(locationId) ?? 0) + 1);
            ...
            try {
                ...
                if (insightError) {
                    ...
                    if (review.location_id) {
                        errorsByLocation.set(review.location_id, (errorsByLocation.get(review.location_id) ?? 0) + 1);
                        lastErrorByLocation.set(review.location_id, insightError.message ?? "insight upsert failed");
                    }
                ...
                for (const tag of analysis.topics ?? []) {
                    const { data: tagRow, error: tagError } = await supabaseAdmin
                        .from("ai_tags")
                        .upsert({ tag: tag.name, category: tag.category ?? null }, { onConflict: "tag" })
                        .select("id")
                        .maybeSingle();
                    ...
                    tagsUpserted += 1;
                    if (locationId) {
                        tagsByLocation.set(locationId, (tagsByLocation.get(locationId) ?? 0) + 1);
                    }
                }
                if (locationId) {
                    processedByLocation.set(locationId, (processedByLocation.get(locationId) ?? 0) + 1);
                }
                await saveCursor({
                    last_source_time: effectiveUpdateTime,
                    last_review_pk: reviewId
