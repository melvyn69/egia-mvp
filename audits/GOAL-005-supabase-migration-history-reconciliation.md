...
La revue indépendante du Run 5 a rendu `APPROVED FOR INTEGRATION`, puis `APPROVED FOR REVIEW` après la preuve du gate. Elle confirme la baseline schema-only, la séparation 97/prospective, l’append-only base-aware, le workflow exécuté depuis la base de confiance, l’import du commit de base pour les forks, le guard lock, le bootstrap fail-closed et les suites `29/29` et `10/10`.

Le lot revu a été intégré en fast-forward strict par la PR #32 au commit `978bfb5ed9e39435f168d0cc89a7480633b015e4`. La protection de `main` exige désormais une branche à jour, les checks `build` et `migration-history-guard` liés à l’app GitHub Actions, y compris pour les administrateurs, ainsi qu’un historique linéaire ; force-push et suppression de branche sont interdits.

La première exécution réelle de la PR #33 a réussi, mais a révélé une annotation de runtime Node 20 sur `actions/checkout@v4` et `actions/setup-node@v4`. La fusion a été suspendue. Après vérification des releases officielles, les deux workflows ont été migrés vers `checkout@v6` et `setup-node@v6`; `checkout@v7` n’a pas été retenu car cette version bloque explicitement le checkout d’une PR de fork sous `pull_request_target`. La revue indépendante a rendu `APPROVED FOR ADMINISTRATIVE OVERRIDE`.

Le guard lock interdisant volontairement à une PR de modifier son propre workflow de confiance, seuls les checks requis ont été retirés temporairement ; toutes les autres protections sont restées actives. Le commit revu `af84ecbe965406196c17fb640e8f6242f7d9c4f3` a été poussé en fast-forward, puis les checks stricts ont été restaurés immédiatement et liés à l’app GitHub Actions. Après rebase, la PR #33 a réussi avec `CI / build` en `1m6s` et `migration-history-guard` en `13s`, sans annotation de runtime Actions. L’unique annotation du build reste le warning lint préexistant et hors scope dans `src/services/coach/useCoachResult.ts:227`.
