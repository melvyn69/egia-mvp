...
Le candidat applicatif GOAL-007 est figé au SHA `baaf5eac5d767722c15c63c4ca799ab5c79e67a7`. Tout descendant autorisé est exclusivement documentaire. Ce SHA inclut la gestion portable et fail-closed de la fermeture anticipée des pipes OpenSSL constatée par la CI Linux; la revue Apple a reconfirmé `APPROVED`. Le premier gel `198d82c0d9fad0134f8400d6c2e03ceec9d1eeb2` est superseded et conservé uniquement comme Evidence historique.

La cause CI était limitée au transport local du test CMS : OpenSSL ferme volontairement une entrée sur un cas négatif et Node recevait `ECONNRESET`; le multiplexage stdin/fd3 restait en outre intermittent sur Linux. Le correctif utilise deux pipes hérités explicites, fd3 pour la signature et fd4 pour le manifeste, avec chemins `/proc/self/fd` sous Linux, listeners d'erreur de fermeture anticipée et statut de sortie OpenSSL toujours autoritatif. Le contrat cryptographique et ses critères restent inchangés.
