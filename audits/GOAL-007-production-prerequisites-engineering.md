...
Le candidat applicatif GOAL-007 est figé au SHA `e6958b647d50c3690cb147a5df8eaa2fdf3136f9`. Tout descendant autorisé est exclusivement documentaire. Ce SHA inclut la gestion portable et fail-closed de la fermeture anticipée des pipes OpenSSL constatée par la CI Linux; la revue Apple a reconfirmé `APPROVED`. Le premier gel `198d82c0d9fad0134f8400d6c2e03ceec9d1eeb2` est superseded et conservé uniquement comme Evidence historique.

La cause CI était limitée au transport local du test CMS : Node fournit des sockets bidirectionnels pour ses stdio et OpenSSL ne peut pas les rouvrir avec `fopen` sous Linux (`ENXIO`); sur un cas négatif, la fermeture anticipée produisait aussi `ECONNRESET`. Le correctif transmet la signature via le stdin natif `-in -` et le manifeste via un FIFO éphémère sans octet persistant, ouvert en écriture non bloquante uniquement lorsque le lecteur OpenSSL existe. Les fermetures anticipées sont absorbées, le statut de sortie OpenSSL reste autoritatif et le contrat cryptographique est inchangé.
